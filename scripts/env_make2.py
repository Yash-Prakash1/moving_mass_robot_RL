...
class env():
    ...
    def _setup_env(self,asset_file='robot_mvw.urdf'):
       # initialize gym
        ...
        # gym.rendeFr_all_camera_sensors(sim)
        self.env_origin=torch.tensor(np.array(self.env_origin),device="cuda:0", dtype=torch.float32)
        ...
        self.initial_observation_space = torch.hstack((self.tracked_states_vec,self.des_vel.unsqueeze(2)))
    ...
    def _compute_reward(self):
        ...
        rew_pos = 0.1*(self.position[:,0,0]**2).unsqueeze(1)
        rew_flag = torch.tensor(100*self.flag1, device="cuda:0").unsqueeze(1)
        return rew_speed+rew_torque+rew_ori+rew_flag+rew_pos#torch.sum(torch.bmm(self.weights,(self.tracked_states_vec-self.des_state)),dim=1)#-0.1*torch.norm(self.dof_force[:,:4],dim=1).reshape((self.dof_force.size()[0]),1)
    ...
    def step(self,action=[]):
        ...
        if self.viewer_flag==True:
            self._render()
        return next_obs, reward, done, self.scan_dot_buf.reshape(self.num_envs,-1,1), image, info, self.root_states_vec#self.dof_states_vec[0,:4,1].to("cpu").detach().numpy(), self.dof_states_vec[0,-1,0].to("cpu").detach().numpy()
    ...
    def get_image(self):
        ...
        image = self.image.permute(0,3,1,2)
    ...
    def reset(self,idx=[]):
        ...
        # next_obs = self.mlp_network(n_obs)
        next_obs = n_obs
        return next_obs, self.scan_dot_buf.reshape(self.num_envs,-1,1), image
