...
def step_terrain1(terrain,obstacle_height=1.,obstacle_width=2.):
    ...
    heightfield[int(terrain.width/2):,int(2*terrain.length/5):int(3*terrain.length/5)]=3
    heightfield[int(terrain.width/2):,int(3*terrain.length/5):int(4*terrain.length/5)]=4
    heightfield[int(terrain.width/2):,int(4*terrain.length/5):]=5
    # for i in range(0,400,50):
    # for i in range(0,400,50):
    #     heightfield[i:i+35,:]=-100
...
def sand_dune_terrain(terrain, dune_height=12, dune_length_ratio=1, obstacle_height=20., obstacle_width_ratio=0.1, num_dunes=2):
    ...
    heightfield[:,int(terrain.length/5):int(2*terrain.length/5)] = dune(dune_height, terrain.length/5)
    heightfield[:,int(2*terrain.length/5):int(3*terrain.length/5)] = dune(2*dune_height, terrain.length/5)
    heightfield[:,int(3*terrain.length/5):int(4*terrain.length/5)] = dune(3*dune_height, terrain.length/5)
    heightfield[:,int(4*terrain.length/5):int(terrain.length)] = dune(4*dune_height, terrain.length/5)
    # Create the ledge
    heightfield[int(terrain.width/2):,:int(terrain.length/5)]=10
    heightfield[int(terrain.width/2):,int(terrain.length/5):int(2*terrain.length/5)]=20
    heightfield[int(terrain.width/2):,int(2*terrain.length/5):int(3*terrain.length/5)]=30
    heightfield[int(terrain.width/2):,int(3*terrain.length/5):int(4*terrain.length/5)]=40
    heightfield[int(terrain.width/2):,int(4*terrain.length/5):]=50

    return heightfield

def training_terrain(terrain, pyramid_base = 150, pyramid_height= 5):

    heightfield=np.zeros((terrain.width,terrain.length),dtype='int16')
    def create_pyramid(base, height):
        pyramid = np.zeros((base, base), dtype='int16')
        for i in range(int(height)):
            pyramid[i:base-i, i:base-i] =  5*i 
        return pyramid

    # Create and scatter pyramids
    spacing = pyramid_base
    pyramid1 = create_pyramid(pyramid_base, pyramid_height)
    pyramid2 = create_pyramid(pyramid_base, (2*pyramid_height))
    pyramid3 = create_pyramid(pyramid_base, (3*pyramid_height))
    pyramid4 = create_pyramid(pyramid_base, (4*pyramid_height))
    for x in range(0, int(terrain.width/2 - pyramid_base + 1), spacing):
        for y in range(int(terrain.length/5)-pyramid_base + 1, int(2*terrain.length/5) - pyramid_base + 1, spacing):
            heightfield[x:x+pyramid_base, y:y+pyramid_base] += pyramid1
    for x in range(0, terrain.width - pyramid_base + 1, spacing):
        for y in range(int(2*terrain.length/5)-pyramid_base + 1, int(3*terrain.length/5) - pyramid_base + 1, spacing):
            heightfield[x:x+pyramid_base, y:y+pyramid_base] += pyramid2
    for x in range(0, terrain.width - pyramid_base + 1, spacing):
        for y in range(int(3*terrain.length/5)-pyramid_base + 1, int(4*terrain.length/5) - pyramid_base + 1, spacing):
            heightfield[x:x+pyramid_base, y:y+pyramid_base] += pyramid3
    for x in range(0, terrain.width - pyramid_base + 1, spacing):
        for y in range(int(4*terrain.length/5)-pyramid_base + 1, int(terrain.length) - pyramid_base + 1, spacing):
            heightfield[x:x+pyramid_base, y:y+pyramid_base] += pyramid4
...
def random_uniform_terrain(terrain, min_height=0, max_height=0.1, step=1, downsampled_scale=None,):
    """
...
def discrete_obstacles_terrain(terrain, max_height=0.5, min_size=0, max_size=5, num_rects=25000, platform_size=10.):
    """
...
def convert_heightfield_to_trimesh(height_field_raw, horizontal_scale, vertical_scale, slope_threshold=None):
    ...
    """
    hf = height_field_raw
    # hf = hf.height_field_raw
    num_rows = hf.shape[0]
    num_cols = hf.shape[1]
    # num_rows = hf.height_field_raw.shape[0]
    # num_cols = hf.height_field_raw.shape[1]
